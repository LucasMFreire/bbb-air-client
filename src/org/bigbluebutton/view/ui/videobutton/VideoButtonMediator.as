	...
	public class VideoButtonMediator extends Mediator {
		...
		private function mouseEventClickHandler(e:MouseEvent):void {
			if (userSession.userList.me.hasStream) {
				...
				if (userSession.skipCamSettingsCheck) {
					shareCameraSignal.dispatch(true, userSession.videoConnection.cameraPosition);
				} else {
		...
		private function userChangeHandler(user:User, type:int):void {
			if (user.me) {
				if (userSession.userList.me.hasStream) {
					view.setStyleName("videoButtonStyle topButtonStyle");
				} else {
					view.setStyleName("videoOffButtonStyle topButtonStyle");
				}
			}
		}
	}
